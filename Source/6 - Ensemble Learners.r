...
dTree=rpart(Species~Petal.Length+Petal.Width,data=data[training,],method="class")

#calculate tree accuracy
prediction=predict(dTree,data[-training,],type="class")
confMat=table(data[-training,]$Species,prediction)
...
for(i in 1:length(n)){
    ...
    for(j in 1:n[i]){
        ...
        bag=data[samp,]
        currentClassifiers=c(currentClassifiers,list(rpart(Species~.,data=bag,method="class")))
    }
...
for(a in 1:length(n)){
    ...
    confMat=table(data[-training,]$Species,decision)
    acc=(confMat[1,1]+confMat[2,2])/
        (confMat[1,1]+confMat[1,2]+confMat[2,1]+confMat[2,2])
...
#Part 4
adaIris=data
adaIris$Species=ifelse(data$Species=="setosa",0,1)
for(a in 1:length(n)){
    adaModel=ada(Species~.,adaIris[training,],loss="exponential",type="discrete",iter=n[a])
    pred=predict(adaModel,adaIris[-training,])
    confMat=table(adaIris[-training,]$Species,pred)
    acc=(confMat[1,1]+confMat[2,2])/
        (confMat[1,1]+confMat[1,2]+confMat[2,1]+confMat[2,2])
